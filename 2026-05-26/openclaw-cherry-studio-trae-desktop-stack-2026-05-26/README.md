---
title: "桌面 AI 三层堆栈：OpenClaw × Cherry × Trae"
slug: openclaw-cherry-studio-trae-desktop-stack-2026-05-26
date: 2026-05-26
weekday: 星期二
author: 王灿
category: 桌面 AI / 工作流堆栈
cover: openclaw-cherry-studio-trae-desktop-stack-2026-05-26.png
description: "OpenClaw（374k stars 国际开源 gateway）做大脑、Cherry Studio（国内团队 46k stars 桌面客户端）做日常对话窗口、Trae（字节免费 IDE）做代码补全。三层各司其职，配置一次永久受用。Cherry Studio 是被低估的非编程 task 入口。"
tags: [OpenClaw, Cherry Studio, Trae, 国产 IDE, MCP, 桌面 AI, BYOK, 本地大模型, ClawRouter]
---
...
凌晨 1 点改完一段 React 代码，从 Trae 切到 Cherry Studio 写日报，再切到微信回老板的消息，老板问「这段需求咱们能不能让 AI 帮你梳理一遍」，又切回去开 DeepSeek 网页版，重新登录贴上下文，企业微信弹出测试同学的 bug 报告，需要顺手让 AI 看一眼复现路径——这一夜的工作记录里，工具切换发生了 7 次，每次切换都意味着「新窗口、新会话、新一遍系统提示词、新一份 API Key 配置」。
...
| 客户端 | stars | license | 跨平台 | 中文 | 本地后端 | RAG | MCP |
|---|---|---|---|---|---|---|---|
| Cherry Studio | 46,281 | AGPL-3.0 | Win / Mac / Linux | 默认中文 | 支持 | 内置 | 支持 |
| LobeChat | 77,688 | NOASSERTION | Web / Docker | 多语言 | 支持 | 支持 | 支持 |
...
云端后端：DeepSeek V4-Pro / Kimi K2.6 / 智谱 GLM Coding Plan
```

L1 这一层是配置一次永久受用的关键。API Key 集中在 OpenClaw 的配置文件里，云端 provider 加新模型只改 YAML，L2 和 L3 完全不感知。要给某个项目临时换模型，OpenClaw 支持 per-request header 路由（X-OpenClaw-Provider 指定 provider 名），客户端发请求时带上 header 就能切换；要做成本控制，ClawRouter 的路由策略可以按 token 数自动降级（短请求走本地、长上下文走云端）。
...
openclaw start      # 默认绑定 127.0.0.1:18789
```

Linux 用户去 GitHub releases 下载对应架构的二进制；Windows 用户装 openclaw-windows-node 配套套件（同 organization，531 stars）。装完用 `curl http://127.0.0.1:18789/v1/models` 验证服务起来了。
...
API Key 用环境变量从 `~/.zshrc` 注入：`export DEEPSEEK_API_KEY=sk-xxx`、`export KIMI_API_KEY=sk-xxx`。
...
Trae 设置 → 模型管理 → 添加自定义模型 → baseURL 这里**必须**填 `http://127.0.0.1:18789/v1/chat/completions` 完整接口路径，不能只填 `http://127.0.0.1:18789/v1`。这是社区 CSDN 教程里反复强调的一点——只填域名 Trae 会显示「无响应」，没有报错提示，第一次接的人很容易卡在这里。
...
文章数据均来自截至 2026-05-26 的 GitHub API 实查、各项目 README / release notes 原文、官方文档与公开使用心得（少数派 / 知乎专栏 / Linux.do / FOSS Force / 阮一峰博客等）。OpenClaw 是 374k+ stars 的国际开源项目（核心维护者 steipete），不是本文作者私有项目；Cherry Studio 由国内 CherryHQ 团队开源；Trae 由字节跳动出品。三家项目的关系是「可互相组合的开放生态」，本文给出的只是其中一种实践路径。与 Claude Code 协作完成研究、图表生成和审稿，跑过 5 道审核 agent（内容真实性 / 图像质量 / 国内可读性 / 链接有效性 / 14 天去重）。
